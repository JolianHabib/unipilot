    ...
    : IProjectDocumentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var duplicateExists = await dbContext.ProjectDocuments.AnyAsync(
            document =>
                document.AcademicProjectId == command.AcademicProjectId &&
        ...
        await activityService.RecordAsync(
            ...
            ProjectActivityType.DocumentUploaded,
            "PDF uploaded",
            $"{document.OriginalFileName} was uploaded to the project.",
            cancellationToken);
        ...
        await RecordProcessingActivityAsync(
            command.OwnerId,
            document,
            false,
            cancellationToken);
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        return await dbContext.DocumentPages
            .AsNoTracking()
            .Where(page => page.ProjectDocumentId == documentId)
    ...
    public async Task<ProjectDocumentFileResult?> GetFileAsync(
        Guid ownerId,
        Guid academicProjectId,
        Guid documentId,
        CancellationToken cancellationToken = default)
    ...
    public async Task<RetryDocumentProcessingResult> RetryProcessingAsync(
        Guid ownerId,
        Guid academicProjectId,
        Guid documentId,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return new RetryDocumentProcessingResult(
                RetryDocumentProcessingStatus.DocumentNotFailed,
                Map(document));
        }

        var existingPages = await dbContext.DocumentPages
            .Where(page => page.ProjectDocumentId == document.Id)
            .ToListAsync(cancellationToken);

        dbContext.DocumentPages.RemoveRange(existingPages);
        ...
        await ExtractAndSavePagesAsync(document, cancellationToken);
        ...
        var resultStatus =
            document.ProcessingStatus == DocumentProcessingStatus.Ready
                ? RetryDocumentProcessingStatus.Success
                : RetryDocumentProcessingStatus.ProcessingFailed;
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            document.ProcessingStatus = DocumentProcessingStatus.Extracting;
            await dbContext.SaveChangesAsync(cancellationToken);

            await using var pdfStream = await fileStorage.OpenReadAsync(
                document.StorageKey,
                cancellationToken);

            var extractedPages = pdfTextExtractor.Extract(pdfStream);

            foreach (var extractedPage in extractedPages)
            {
                dbContext.DocumentPages.Add(new DocumentPage
                {
        ...
        catch (Exception exception)
        {
            ...
            document.FailureReason = Truncate(exception.Message, 1000);

            await dbContext.SaveChangesAsync(CancellationToken.None);
        }
    }

    private Task RecordProcessingActivityAsync(
        Guid ownerId,
        ProjectDocument document,
        bool isRetry,
        CancellationToken cancellationToken)
    {
        ...
        var type = succeeded
            ? ProjectActivityType.DocumentProcessed
            : ProjectActivityType.DocumentProcessingFailed;
        ...
        var description = succeeded
            ? $"{document.OriginalFileName} is ready. " +
              $"{document.PageCount} page(s) were extracted."
            : $"UniPilot could not process {document.OriginalFileName}.";

        return activityService.RecordAsync(
            ownerId,
            document.AcademicProjectId,
            type,
            title,
    ...
    private Task CreateProcessingNotificationAsync(
        Guid ownerId,
        ProjectDocument document,
        bool isRetry,
        CancellationToken cancellationToken)
    {
        ...
        var type = succeeded ? "Success" : "Error";
        ...
        var message = succeeded
            ? $"“{document.OriginalFileName}” is ready. " +
              $"{document.PageCount} page(s) were extracted."
            : $"UniPilot could not process “{document.OriginalFileName}”. " +
              "You can retry from the project workspace.";
    ...
    private static ProjectDocumentResult Map(ProjectDocument document)
    {
